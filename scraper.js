 ...
 * de autenticación almacenados en variables de entorno.
 *
 * Uso:
 ...
 *   COPEC_FIRMA        -> Firma de autenticacion (header firma)
...
async function main() {
  ...
  const tokens = {
    ...
    firma: process.env.COPEC_FIRMA
  };
  const cuentaId = parseInt(process.env.COPEC_CUENTA_ID);

  if (!tokens.access_token || !tokens.firma || !cuentaId) {
    throw new Error('Variables COPEC_ACCESS_TOKEN, COPEC_FIRMA y COPEC_CUENTA_ID son requeridas');
  }
